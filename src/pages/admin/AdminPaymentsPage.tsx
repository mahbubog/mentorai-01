  ...
  const handleApprove = async (paymentId: string, courseId: string, userId: string) => {
    ...
    try {
      ...
      const { error: paymentError } = await supabase
        .from('payments')
        .update(paymentUpdate as PaymentsUpdate) // Use specific Update type
        .eq('id', paymentId);
      ...
      const { error: enrollmentError } = await supabase.from('enrollments').insert(enrollmentData as EnrollmentsInsert); // Use specific Insert type
      ...
      await supabase.from('notifications').insert(notificationData as NotificationsInsert); // Use specific Insert type
